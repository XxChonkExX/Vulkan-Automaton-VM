...
#else
// ============================================================================
// Non-Windows: a dlopen-based amdhip64 loader follows (see ioring_backend.cpp
// for the pattern). Until then every entry point is a safe no-op and create()
// returns nullptr - the factory degrades gracefully.
// ============================================================================
...
std::unique_ptr<HipMemoryBackend> HipMemoryBackend::create(int deviceIndex) {
    (void)deviceIndex;
    VVM_LOG_WARN("hip backend: not available on this platform yet");
    return nullptr;
}

std::vector<BackendHeap> HipMemoryBackend::heaps() const { return {}; }
std::vector<BackendMemType> HipMemoryBackend::memoryTypes() const { return {}; }
BackendBudget HipMemoryBackend::heapBudget(uint32_t) const { return {}; }
bool HipMemoryBackend::type_is_host_visible(uint32_t) const { return false; }
BackendMemory HipMemoryBackend::allocate(const BackendAllocRequest&, int* e) {
    if (e) *e = encode_backend_error(-1);
    return 0;
}
void HipMemoryBackend::free(BackendMemory) {}
void* HipMemoryBackend::map(BackendMemory, bool* ok) { if (ok) *ok = false; return nullptr; }
void HipMemoryBackend::unmap(BackendMemory) {}
BackendBuffer HipMemoryBackend::create_buffer(BackendMemory, uint64_t, uint64_t,
                                              uint64_t, bool, int* e) {
    if (e) *e = encode_backend_error(-1);
    return 0;
}
bool HipMemoryBackend::bind_buffer(BackendBuffer, BackendMemory) { return false; }
