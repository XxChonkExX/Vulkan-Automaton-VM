// CudaMemoryBackend smoke test: dynamic loader + allocate/free/budget plane,
// PLUS a full UnifiedMemoryPool created over the CUDA backend (backend-driven
// discovery path). Mirrors hip_backend_test.cpp. Build: cl
// cuda_backend_test.cpp (links the vulkan_vm import lib; nvcuda itself is
// loaded dynamically at runtime - no toolkit needed to build OR run this).
...
int main() {
    ...
    // 1. Factory: CUDA via dynamic loader. Device 0 = the sole NVIDIA GPU.
    std::unique_ptr<CudaMemoryBackend> cuda = CudaMemoryBackend::create(0);
    if (!cuda) {
        std::printf("SKIP: no CUDA device (nvcuda absent or no NVIDIA GPU)\n");
        return 2;
    }
    std::printf("ok: cuda backend created (%s)\n", cuda->name());

    // 2. Discovery
    const auto heaps = cuda->heaps();
    const auto types = cuda->memoryTypes();
    std::printf("ok: %zu heap(s), %zu type(s)\n", heaps.size(), types.size());
    if (heaps.empty() || types.empty()) { std::printf("FAIL: empty discovery\n"); ++failures; }

    // 3. Live budget
    const BackendBudget b = cuda->heapBudget(0);
    ...
    // 4. Allocate / echo / free cycle (three sizes)
    ...
    for (const Case& c : cases) {
        int err = 0;
        const BackendMemory mem = cuda->allocate({c.size, 0, false, false, 0.0f, 0}, &err);
        if (mem == 0) {
            std::printf("FAIL: allocate %s (err %d)\n", c.name, err);
            ++failures;
            continue;
        }
        const BackendBuffer buf = cuda->create_buffer(mem, 0, c.size,
                                                      VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
                                                      false, &err);
        if (buf != mem) {   // CUDA: buffer echoes the pointer
            std::printf("FAIL: buffer echo for %s\n", c.name);
            ++failures;
        }
        const uint64_t addr = cuda->buffer_device_address(buf);
        if (addr != mem) {
            std::printf("FAIL: device address echo for %s\n", c.name);
            ++failures;
        }
        void* mapped = cuda->map(mem, nullptr);
        if (mapped != nullptr) {
            std::printf("FAIL: device memory should not map\n");
            ++failures;
        }
        cuda->destroy_buffer(buf);   // no-op: ownership follows memory
        cuda->free(mem);
        std::printf("ok: %s allocate/echo/free\n", c.name);
    }

    // 5. Budget reflects the freed 1 GiB (allocation returned)
    const BackendBudget after = cuda->heapBudget(0);
    std::printf("ok: budget after frees %llu MB used\n",
                (unsigned long long)(after.usedBytes / (1024 * 1024)));

    // 6. Factory dispatch: explicit Cuda kind routes to this backend.
    DeviceConfig cudaCfgProbe{};
    cudaCfgProbe.backendDeviceIndex = 0;
    cudaCfgProbe.memBackendKind = static_cast<int32_t>(MemBackendKind::Cuda);
    auto viaFactory = create_memory_backend(MemBackendKind::Cuda, cudaCfgProbe);
    std::printf("%s: factory Cuda kind -> %s\n",
                viaFactory ? "ok" : "FAIL", viaFactory ? viaFactory->name() : "(null)");
    if (!viaFactory) ++failures;

    // 7. FULL POOL over the CUDA backend: backend-driven discovery, buddy
    //    sub-allocation, budget policy - the end-to-end gate.
    DeviceConfig cudaCfg{};
    cudaCfg.backendDeviceIndex = 0;
    cudaCfg.memBackendKind = static_cast<int32_t>(MemBackendKind::Cuda);
    PoolConfig pcfg;
    pcfg.blockSize = 64ull * 1024ull * 1024ull;      // small blocks for the test
    pcfg.maxBlocks = 4;
    pcfg.enableHostVisible = false;
    auto pool = UnifiedMemoryPool::create(cudaCfg, pcfg);
    if (!pool.has_value()) {
        std::printf("FAIL: pool create over CUDA backend\n");
        ++failures;
    } else {
        std::printf("ok: UnifiedMemoryPool created over CUDA backend\n");
        vvm::AllocDesc desc;
        ...
        // Budget reservation (KV pattern): hold 1 GiB, verify small allocs
        // still land, release (double-release saturates), land the giant
        // dedicated, verify clean teardown.
    ...
    if (failures == 0) {
        std::printf("ALL CUDA BACKEND TESTS PASSED\n");
        return 0;
    }
