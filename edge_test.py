...
from chonk import build_lora_chonk_setup, reset_chonk_cache, patch_linear_cache_for_chunked_training, install_chonk_allocator

CHUNK = int(sys.argv[1])
SEQ = int(sys.argv[2])
RESULT = sys.argv[3]

if os.environ.get('CHONK_ALLOCATOR', '1') == '1':
...
setup = build_lora_chonk_setup(
    ...
    attn_implementation=os.environ.get('CHONK_ATTN', 'eager'),
)
pool = setup['pool']; kv_cache = setup['kv_cache']; model = setup['model']
model.train()
patch_linear_cache_for_chunked_training()

CHONK_PAUSE = float(os.environ.get('CHONK_PAUSE', '0.02'))
...
for i in range(n_chunks):
    ...
    try:
        ...
        pass  # pluggable allocator has no peak tracking; use pool stats
    ...
    if i % 4 == 3:
        ...
        s = pool.stats()
        print(f'  chunk {i+1}/{n_chunks} ok ({time.time()-t0:.0f}s, pool_used={s["totalUsed"]/1e9:.1f}GB, blocks={s["blockCount"]})', flush=True)
