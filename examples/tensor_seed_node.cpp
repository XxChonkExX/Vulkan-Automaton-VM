...
int main() {
    ...
    std::cout << "=== VulkanVM Tensor Seed Node ===\n";
    std::cout << "Listening on 0.0.0.0:51001 for Windows client...\n\n";

    // Initialize Vulkan
    VkApplicationInfo appInfo{};
    appInfo.sType = VK_STRUCTURE_TYPE_APPLICATION_INFO;
    appInfo.pApplicationName = "VulkanVM Tensor Seed Node";
    ...
    // Create tensor transport with NetworkOnly preference
    vvm::tensor::TransportConfig transportCfg;
    transportCfg.preference = vvm::tensor::TransportConfig::Preference::NetworkOnly;
    transportCfg.networkPort = 51001;
    transportCfg.enableAsyncPipeline = false;

    auto transport = vvm::tensor::Transport::create(transportCfg, {devCfg}, poolCfg);
    if (!transport) { std::cerr << "FAIL: create transport\n"; return 1; }

    if (!transport->initialize()) { std::cerr << "FAIL: initialize transport\n"; return 1; }

    // Join cluster as bootstrap (seed) node
    if (!transport->joinCluster("192.168.0.117:51001")) { 
        std::cerr << "FAIL: joinCluster as bootstrap\n"; return 1; 
    }

    std::cout << "\nTensor seed node running on 192.168.0.117:51001\n";
    std::cout << "Local node ID: " << transport->getLocalNodeId() << "\n";
    std::cout << "Waiting for Windows client to connect...\n";
    std::cout << "Press Ctrl+C to stop\n\n";

    while (g_running) {
        std::this_thread::sleep_for(std::chrono::seconds(2));
        std::cout << "\rRunning... (local node: " << transport->getLocalNodeId() << ")" << std::flush;
    }
