...
VVM_API std::vector<uint8_t> serializeNodeId(const NodeId& id);
bool deserializeNodeId(const uint8_t*& p, const uint8_t* end, NodeId& out);

VVM_API std::vector<uint8_t> serializeNodeInfo(const NodeInfo& info);
VVM_API bool deserializeNodeInfo(const uint8_t*& p, const uint8_t* end, NodeInfo& out);

VVM_API std::vector<uint8_t> serializeNodeList(const std::vector<NodeInfo>& list);
VVM_API bool deserializeNodeList(const std::vector<uint8_t>& data, std::vector<NodeInfo>& out);
